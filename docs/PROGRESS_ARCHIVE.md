...
- **Deployed V1.0.6 — Desktop jumped 90-92, Mobile stayed 67-68.** Real PageSpeed re-check after the Home-eager deploy showed a big desktop win but no mobile movement (LCP still 5.8s, "LCP request discovery" still 0). Ran a second LLM-council review on whether to add `<link rel="preload">` for the hero — council flagged that nobody had confirmed which element (image or video) is the actual measured LCP resource. Confirmed via the browser's `PerformanceObserver` API directly against production: the LCP element is the `<video>`, but the measured *resource* is its poster (`hero.jpg`), not `hero.mp4`. Added `<link rel="preload" as="image" href="/assets/images/home/hero.jpg" fetchpriority="high">` to `index.html` only — deliberately no video preload (would compete for mobile bandwidth against the actual measured resource). `test:visual` 34/34, confirmed via `vite preview` that `hero.jpg` now loads before the JS bundle, mobile 375px clean. **Deployed V1.0.7.**
- **Switched to Google's own `pagespeed.web.dev` as source of truth** — Hostinger's PageSpeed panel is a third-party proxy with different numbers than Google's real tool. Re-ran on the live V1.0.7: Mobile **69** (up from Hostinger's 67-68), LCP 5.3s (down from 5.8s), and — the real proof the preload worked — **"LCP request discovery" no longer appears as a failing diagnostic at all**. Remaining top issues are different in kind: cache lifetimes (3.2 MB potential savings, the biggest one) and overall payload size (3.7 MB) — no longer a JS-discovery-order problem.
- **Added local Lighthouse tooling:** `lighthouse` devDependency + `npm run lighthouse [mobile|desktop]` (`scripts/lighthouse.sh`) — builds, serves via `vite preview`, runs a real audit, saves HTML to gitignored `lighthouse-reports/`. Fixed two Windows-specific bugs found while testing it (npx-wrapper-PID kill bug, chrome-launcher EPERM cleanup crash). Confirmed local numbers are NOT comparable to `pagespeed.web.dev` (54 locally vs. 69 on Google's tool, same build) — documented as a relative-comparison-only tool.
...
**Task 26 / UX-11 (XS) — hecho.** `Footer.jsx:102,105` (columna "Aviso Importante"): las dos `<p>` del disclaimer bilingüe ES/EN `text-[11px]` → `text-[12px]`; `leading-relaxed` ya estaba. Cumple `DESIGN.md` §3/§9 ("Minimum disclaimer text: 12px"). Verificado en browser `:3000` — `getComputedStyle` = `12px` / line-height 19.5px, la caja del aviso sigue encajando en su columna, sin salto de layout. **`test:visual` suite completa: 21 passed, 1 failed = el fallo pre-existente conocido `nosotros-viewport` desktop-chrome (placeholder `about/hero.jpg`), no relacionado.** Cero diff por este cambio: ningún snapshot encuadra el footer. Commit `36c1931`.

**Task 24 / CPY-05 (XS) — hecho.** `src/data/treatmentPages.js:478` (`manchas-cicatrices` `problemContextBody`): `las marcas permanentes tras imperfecciones` → `las marcas persistentes tras imperfecciones`. Contexto = descripción del estado del problema, no del resultado; "persistentes" es más conservador. Sin palabra prohibida. Verificado en browser `/faciales/manchas-cicatrices`; `git grep "marcas permanentes"` = 0. `test:visual` saltado por gate DoD. Commit `6c634c3`.

**Task 22 / SEO-08 — cerrada como no-op (sin cambio de código).** Reconciliada contra `project_prf_content_strategy` + `project_seo_backlog_89_810_prf_positioning` + ciclo de naming canónico (`DECISIONS.md` 2026-08-28) + grep del árbol actual. Detalle y "how to apply": `DECISIONS.md` 2026-08-31. Doc-only.
...
- **`home/founder.jpg` + `.webp`** se borraron en el commit siguiente (`fa791b2`, `chore(assets)`) — quedaban sin referencias de código. (La entrada original decía "pendiente, bloqueado por classifier"; se resolvió en el mismo día.)
...
- **Sin cambio de estructura.** La sección se mantiene tal cual (`spotlightRow` imagen+texto, link `Conoce más sobre Nancy →`) — link-driver a la bio casi huérfana + anchor de E-E-A-T de `/nosotros`. Solo cambió el asset. La reducción a "founder-primer compacto" quedó como follow-up opcional, NO ejecutada.
- **Verificación.** `:3000`: pinta el retrato, `.webp` 200, distinto del crop del hero. `test:visual`: re-baseline intencional de `nosotros-founder-with-link` desktop + mobile; 34/34 tras update. WCAG AA OK.
...
- **Verificación.** `:3000`: video autoplaya muted+loop, still en 2º slot, sección 1350px (= oxigenoterapia). `test:visual` 34/34 sin diffs. Compliance OK.
...
- **Verificación:** navegación real en `:3000` (`/` → click "Contacto" → `/contacto` por SPA ✓). `npm run test:visual` 34/34 sin diffs.
- Debugging: `superpowers:systematic-debugging` — root cause = CTAs nunca cableados.
