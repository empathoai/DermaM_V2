...
| 8.18 | **NUEVO (auditoría 2026-08-26):** `aggregateRating` `4.9`/`117` hardcodeado en JSON-LD sin que la página muestre esas reviews. Política de Google exige valoraciones genuinas y visibles en la misma página → riesgo de acción manual / pérdida de rich results del dominio + honestidad (`MEDICAL_COMPLIANCE.md`). | `Home.jsx:51-55`, `Contacto.jsx:98-101` | Alta | **En curso (2026-08-27)** — decidido con usuario: **(b) eliminar `aggregateRating`** de ambos archivos en el ciclo del cluster de schema del `#organization` (sub-proyecto A). Se reintroduce respaldado por reseñas visibles en el ítem 8.20. `grep`: `aggregateRating` no lo lee ningún componente — solo vive en los 2 `<script>` JSON-LD, quitarlo es riesgo cero. |
...
| 8.17 | No hay Google Search Console ni GA4 configurados/conectados. **Ampliado 2026-08-26:** agregar **Bing Webmaster Tools "AI Performance"** — única analítica de citación IA que existe (citas + grounding queries; feb 2026). Sin esto no se valida 8.11/8.13/8.15/8.16. | Fuera del repo (config externa) | Alta | Pendiente — **PRIORIDAD 4, desbloquea el resto. No toca código.** |
