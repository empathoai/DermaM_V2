export const contactFaq = {
  headline: 'PREGUNTAS FRECUENTES',
  items: [
    {
      question: '¿Necesito cita previa?',
      answer: 'Sí. Te recomendamos agendar tu cita antes de visitarnos para asegurar disponibilidad y brindarte una atención personalizada.'
    },
    {
      question: '¿Puedo consultar por WhatsApp?',
      answer: 'Sí. Puedes escribirnos por WhatsApp al +1 561 253 5384 y nuestro equipo te orientará.'
    },
    {
      question: '¿Dónde están ubicados?',
      answer: 'Nuestra ubicación está en 5707 S Dixie Hwy UNIT D, West Palm Beach, FL 33405.'
    },
    {
      question: '¿Qué servicios puedo consultar?',
      answer: 'Puedes consultar sobre valoración facial, acné, manchas, cicatrices, rejuvenecimiento facial, limpiezas faciales y otros tratamientos personalizados.'
    },
    {
      question: '¿Atienden en español e inglés?',
      answer: 'Sí. Nuestro equipo te atiende en español e inglés, tanto en nuestro medical spa en West Palm Beach como por WhatsApp y teléfono.'
    },
    {
      question: '¿Cómo elijo un buen med spa en West Palm Beach?',
      answer: 'Antes de reservar, fíjate en tres cosas: que el personal tenga licencia y que sepas quién realiza cada tratamiento, que te hagan una valoración de la piel antes de recomendarte nada, y que te expliquen el procedimiento y los cuidados posteriores en tu idioma. En DERMA.M te atendemos en español e inglés; Nancy Nieto, fundadora y directora, es flebotomista certificada en EE.UU. y especialista en estética facial con licencia del Estado de Florida, y realiza la valoración inicial. Te acompañamos antes y después de cada tratamiento en nuestro medical spa en West Palm Beach.'
    }
  ]
};
